...
class MyDirectIntegrator(SamplingIntegrator):
    ...
    def integrator_sample(self, scene, sampler, rays, medium, active=True):
        ...
        result = dr.select(active, \
            emitter_vis.eval(si, active), mi.Vector3f(0.0))
        ...
        mis = dr.select(ds.delta, mi.Float(1), self.mis_weight(ds.pdf, bsdf_pdf))
        result += dr.select(active_e, emitter_val * bsdf_val * mis, mi.Vector3f(0))
        ...
        # ds.object = emitter
        emitter_pdf = dr.select(delta, mi.Float(0), scene.pdf_emitter_direction(si, ds, active_b))
        result += dr.select(active_b, bsdf_val * emitter_val * self.mis_weight(bs.pdf, emitter_pdf), mi.Vector3f(0))
